...
Class SQLiteDB {
    ...
    __New() {
        ...
        If (SQLiteDB._RefCount = 0) {
            ...
            LibVersion := StrGet(DllCall(SQLiteDB._SQLiteDLL "\sqlite3_libversion", "Cdecl UPtr"), "UTF-8")
    ...
    OpenDB(DBPath, Access := "W", Create := True) {
        ...
        HDB := 0
        RC := DllCall(SQLiteDB._SQLiteDLL "\sqlite3_open_v2", "Ptr", UTF8, "UPtrP", &HDB, "Int", Flags, "Ptr", 0, "Cdecl Int")
        If (RC) {
            This._Path := ""
    ...
    CloseDB() {
        ...
        For Each, Stmt in This._Stmts
            DllCall(SQLiteDB._SQLiteDLL "\sqlite3_finalize", "Ptr", Stmt, "Cdecl Int")
        If (RC := DllCall(SQLiteDB._SQLiteDLL "\sqlite3_close", "Ptr", This._Handle, "Cdecl Int"))
    ...
    Exec(SQL, Callback := "") {
        ...
        RC := DllCall(SQLiteDB._SQLiteDLL "\sqlite3_exec", "Ptr", This._Handle, "Ptr", UTF8, "Int", CBPtr, "Ptr", ObjPtr(This),
            "UPtrP", &Err, "Cdecl Int")
        If (CBPtr)
            CallbackFree(CBPtr)
        If (RC) {
            ...
            This.ErrorCode := RC
            DllCall(SQLiteDB._SQLiteDLL "\sqlite3_free", "Ptr", Err, "Cdecl")
            Return False
        }
    ...
    GetTable(SQL, &TB, MaxResult := 0) {
        ...
        RC := DllCall(SQLiteDB._SQLiteDLL "\sqlite3_get_table", "Ptr", This._Handle, "Ptr", UTF8, "UPtrP", &Table,
        ...
        TB.RowCount := Rows
        If (MaxResult = -1) {
            DllCall(SQLiteDB._SQLiteDLL "\sqlite3_free_table", "Ptr", Table, "Cdecl")
            Return True
        }
        If (MaxResult = -2)
            GetRows := 0
    ...
    Prepare(SQL, &ST) {
        ...
        Local Stmt := 0
        Local UTF8 := This._StrToUTF8(SQL)
        RC := DllCall(SQLiteDB._SQLiteDLL "\sqlite3_prepare_v2", "Ptr", This._Handle, "Ptr", UTF8, "Int", -1,
            "UPtrP", &Stmt, "Ptr", 0, "Cdecl Int")
        If (RC)
            Return This._SetError(RC)
        ColumnNames := []
        ColumnCount := DllCall(SQLiteDB._SQLiteDLL "\sqlite3_column_count", "Ptr", Stmt, "Cdecl Int")
        If (ColumnCount > 0) {
            ColumnNames.Length := ColumnCount
            Loop ColumnCount {
                Pointer := DllCall(SQLiteDB._SQLiteDLL "\sqlite3_column_name", "Ptr", Stmt, "Int", A_Index - 1, "Cdecl UPtr")
        ...
        ST.ParamCount := DllCall(SQLiteDB._SQLiteDLL "\sqlite3_bind_parameter_count", "Ptr", Stmt, "Cdecl Int")
        ST._Handle := Stmt
        ST._DB := This
    ...
    CreateScalarFunc(Name, Args, Func, Enc := 0x0801, Param := 0) {
        ...
        If !(This._Handle)
            Return This._SetError(0, "Invalid database handle!")
        Local RC := DllCall(SQLiteDB._SQLiteDLL "\sqlite3_create_function", "Ptr", This._Handle, "AStr", Name, "Int", Args,
    ...
    EnableLoadExtension(Enable := 1) {
        Local RC := DllCall(SQLiteDB._SQLiteDLL "\sqlite3_db_config", "Ptr", This._Handle, "Int", 1005, "Int", !!Enable,
            "Ptr", 0, "Cdecl Int")
        Return (RC) ? This._SetError(RC) : True
    }
    ; ===================================================================================================================
    ...
    LoadExtension(File, Proc?) {
        Local RC := IsSet(Proc) ? DllCall(SQLiteDB._SQLiteDLL "\sqlite3_load_extension", "Ptr", This._Handle, "AStr", File,
            "AStr", Proc, "Ptr", 0, "Cdecl Int")
            : DllCall(SQLiteDB._SQLiteDLL "\sqlite3_load_extension", "Ptr", This._Handle, "AStr", File,
    ...
    LastInsertRowID(&RowID) {
        ...
        If !(This._Handle)
            Return This._SetError(0, "Invalid database handle!")
        RowID := DllCall(SQLiteDB._SQLiteDLL "\sqlite3_last_insert_rowid", "Ptr", This._Handle, "Cdecl Int64")
        Return True
    }
    ; ===================================================================================================================
    ...
    TotalChanges(&Rows) {
        ...
        If !(This._Handle)
            Return This._SetError(0, "Invalid database handle!")
        Rows := DllCall(SQLiteDB._SQLiteDLL "\sqlite3_total_changes", "Ptr", This._Handle, "Cdecl Int")
        Return True
    }
    ; ===================================================================================================================
    ...
    SetTimeout(Timeout := 1000) {
        ...
        If !IsInteger(Timeout)
            Timeout := 1000
        If (RC := DllCall(SQLiteDB._SQLiteDLL "\sqlite3_busy_timeout", "Ptr", This._Handle, "Int", Timeout, "Cdecl Int"))
            Return This._SetError(RC)
        Return True
    }
    ; ===================================================================================================================
    ...
    EscapeStr(&Str, Quote := True) {
        ...
        Local Ptr := DllCall(SQLiteDB._SQLiteDLL "\sqlite3_mprintf", "Ptr", OP, "Ptr", UTF8, "Cdecl UPtr")
        Str := StrGet(Ptr, "UTF-8")
        DllCall(SQLiteDB._SQLiteDLL "\sqlite3_free", "Ptr", Ptr, "Cdecl")
        Return True
    }
    ; ===================================================================================================================
    ...
    ExtErrCode() {
        If !(This._Handle)
            Return 0
        Return DllCall(SQLiteDB._SQLiteDLL "\sqlite3_extended_errcode", "Ptr", This._Handle, "Cdecl Int")
    }
    ; ===================================================================================================================
    ; PRIVATE _Changes
    ; ===================================================================================================================
    _Changes() {
        Return DllCall(SQLiteDB._SQLiteDLL "\sqlite3_changes", "Ptr", This._Handle, "Cdecl Int")
    }
    ; ===================================================================================================================
    ; PRIVATE _ErrMsg
    ; ===================================================================================================================
    _ErrMsg() {
        Local RC
        If (RC := DllCall(SQLiteDB._SQLiteDLL "\sqlite3_errmsg", "Ptr", This._Handle, "Cdecl UPtr"))
    ...
    ; ===================================================================================================================
    _ErrCode() {
        Return DllCall(SQLiteDB._SQLiteDLL "\sqlite3_errcode", "Ptr", This._Handle, "Cdecl Int")
    }
    ; ===================================================================================================================
    ; PRIVATE _ErrStr
    ; ===================================================================================================================
    _ErrStr(ErrCode) {
        Return StrGet(DllCall(SQLiteDB._SQLiteDLL "\sqlite3_errstr", "Int", ErrCode, "Cdecl UPtr"), "UTF-8")
    }
    ; ===================================================================================================================
    ; PRIVATE _SetError
    ; ===================================================================================================================
    ...
    ; +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    Static Version := ""
    Static _SQLiteDLL := A_ScriptDir . (A_PtrSize = 4 ? "\SQLite3.dll" : "\SQLite3_x64.dll")
    Static _RefCount := 0
    ...
    Class _Prepared {
                ...
                For ParamType, Value In Param {
                    ...
                    Switch ParamType {
                        Case "Blob":
                            ...
                            RC := DllCall(SQLiteDB._SQLiteDLL "\sqlite3_bind_blob", "Ptr", This._Handle, "Int", Index, "Ptr", Value,
                        ...
                        Case "Double":
                            ...
                            RC := DllCall(SQLiteDB._SQLiteDLL "\sqlite3_bind_double", "Ptr", This._Handle, "Int", Index, "Double", Value,
                                "Cdecl Int")
                            If (RC)
                                Return This._SetError(RC)
                        Case "Int":
                            ...
                            RC := DllCall(SQLiteDB._SQLiteDLL "\sqlite3_bind_int", "Ptr", This._Handle, "Int", Index, "Int", Value,
                                "Cdecl Int")
                            If (RC)
                                Return This._SetError(RC)
                        Case "Int64":
                            ...
                            RC := DllCall(SQLiteDB._SQLiteDLL "\sqlite3_bind_int64", "Ptr", This._Handle, "Int", Index, "Int64", Value,
                                "Cdecl Int")
                            If (RC)
                                Return This._SetError(RC)
                        Case "Null":
                            RC := DllCall(SQLiteDB._SQLiteDLL "\sqlite3_bind_null", "Ptr", This._Handle, "Int", Index, "Cdecl Int")
                            If (RC)
                                Return This._SetError(RC)
                        Case "Text":
        ...
        Step(Row?) { ; !!!!! Note: If Row is not omitted is must be a VarRef !!!!!
            ...
            Loop RC {
                ...
                Switch ColumnType {
                    ...
                    Case SQLITE_INTEGER:
                        Value := DllCall(SQLiteDB._SQLiteDLL "\sqlite3_column_int64", "Ptr", This._Handle, "Int", Column, "Cdecl Int64")
                        Res[A_Index] := Value
                    Case SQLITE_FLOAT:
                        Value := DllCall(SQLiteDB._SQLiteDLL "\sqlite3_column_double", "Ptr", This._Handle, "Int", Column, "Cdecl Double")
                        Res[A_Index] := Value
                    Case SQLITE_NULL:
                        Res[A_Index] := ""
                    Default:
                        Value := DllCall(SQLiteDB._SQLiteDLL "\sqlite3_column_text", "Ptr", This._Handle, "Int", Column, "Cdecl UPtr")
        ...
        Reset(ClearBindings := True) {
            ...
            If !(This._Handle)
                Return This._SetError(0, "Invalid query handle!")
            If (RC := DllCall(SQLiteDB._SQLiteDLL "\sqlite3_reset", "Ptr", This._Handle, "Cdecl Int"))
                Return This._SetError(RC)
            If (ClearBindings) && (RC := DllCall(SQLiteDB._SQLiteDLL "\sqlite3_clear_bindings", "Ptr", This._Handle, "Cdecl Int"))
                Return This._SetError(RC)
            This.CurrentStep := 0
            Return True
        }
        ...
        Free() {
            ...
            If !(This._Handle)
                Return True
            If (RC := DllCall(SQLiteDB._SQLiteDLL "\sqlite3_finalize", "Ptr", This._Handle, "Cdecl Int"))
                Return This._SetError(RC)
...
SQLiteDB_RegExp(Context, ArgC, Values) {
    ...
    If (ArgC = 2) {
        AddrN := DllCall(SQLiteDB._SQLiteDLL "\sqlite3_value_text", "Ptr", NumGet(Values + 0, "UPtr"), "Cdecl UPtr")
        AddrH := DllCall(SQLiteDB._SQLiteDLL "\sqlite3_value_text", "Ptr", NumGet(Values + A_PtrSize, "UPtr"), "Cdecl UPtr")
        Result := RegExMatch(StrGet(AddrH, "UTF-8"), StrGet(AddrN, "UTF-8"))
    }
    DllCall(SQLiteDB._SQLiteDLL "\sqlite3_result_int", "Ptr", Context, "Int", !!Result, "Cdecl") ; 0 = false, 1 = true
}